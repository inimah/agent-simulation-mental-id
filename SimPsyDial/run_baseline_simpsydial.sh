...
#
# Combines:
#   - Code structure from simulator/run_simulation.sh and simulate_conversation.py
#       · Ollama server lifecycle management (start, health-check, preflight warmup)
#       · User speaks first; shared_history uses "user"/"assistant" roles directly
#       · _strip_actions sanitizer + user farewell detection + counselor closing detection
#   - User-agent prompt from simulator/simulate_conversation.py
#       · make_user_system_prompt() with USER_AGENT_TYPES + _USER_AGENT_STYLE variation
...
#
# Work list: loaded from data/simulated-sft-fin/ (155 unique scenario/emotion/agent triples)
# so output dialogs are directly comparable to the sft-fin reference corpus.
#
# Output dir: data/simulated-sft_baseline_v4
# Output filenames: baseline_v4_{scenario_id:04d}_{emotion}_{agent_type}_{run_ts}.json
#
# Usage:
#   bash run_baseline_simpsydial.sh [extra args passed to simulate_baseline_v4.py]
#   bash run_baseline_simpsydial.sh --resume
#   bash run_baseline_simpsydial.sh --scenario-ids 1 2 3
#   bash run_baseline_simpsydial.sh --dry-run
#   bash run_baseline_simpsydial.sh --manifest-dir /path/to/other/reference/dir

set -euo pipefail

# ── Paths ──────────────────────────────────────────────────────────────────────
SCRIPT_DIR="${SLURM_SUBMIT_DIR:-PROJECT_ROOT/SimPsyDial"
...
OUTPUT_DIR="${PROJECT_ROOT}/agentic_curriculum/data/simulated-sft_baseline_v4"
LOG_DIR="${PROJECT_ROOT}/agentic_curriculum/logs"

mkdir -p "${LOG_DIR}" "${OUTPUT_DIR}"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
LOG_FILE="${LOG_DIR}/baseline_v4_${TIMESTAMP}.log"
...
python3 "${SCRIPT_DIR}/simulate_baseline_v4.py" \
    --xlsx        "${XLSX}" \
