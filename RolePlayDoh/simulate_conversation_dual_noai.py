"""
simulate_conversation_dual_noai.py  —  Baseline D3: Roleplay-doh Dual Adherence (Non-AI-Aware)
-----------------------------------------------------------------------------------------------
Identical to Baseline D2 (simulate_conversation_dual.py) except that NEITHER
agent is told they are interacting with an AI system:
...
Both agents still receive full Roleplay-doh principle-adherence prompting each turn.
Only the AI-identity framing is changed — nothing else.

Output filenames : roleplay_doh_dual_noai_{scenario_id:04d}_{emotion}_{agent_type}_{run_ts}.json
Output directory : data/simulated-sft_baseline_d_roleplaydoh_dual_noai
...
_AGENT_PRINCIPLES: dict[str, list[str]] = {
    ...
    "defensif": [
        "Tunjukkan skeptisisme awal terhadap kemampuan konselor: 'kamu ngerti ga sih masalah aku?'",
...
def _build_backstory(scenario: dict, target_emotion: str) -> str:
    return (
        ...
        "Kamu mengikuti sesi konseling dengan Teta karena ingin curhat atau mencari dukungan."
    )
...
# ── Chatbot system prompt (non-AI-aware — following make_counselor_system_prompt()) ──
...
def make_chatbot_system_prompt(
    ...
    if avoid_phrases:
        ...
        avoid_block = (
            f"\n12. DILARANG mengulang frasa validasi yang sudah dipakai sebelumnya: {phrase_list}. "
    ...
    return f"""Sekarang kamu adalah konselor psikologi virtual bernama Teta. {intro_rule}
...
11. DILARANG: narasi internal atau komentar di luar respons.{avoid_block}
"""
...
def simulate_conversation(
    ...
    dialog_id   = f"roleplay_doh_dual_noai_{scenario_id:04d}_{target_emotion}_{int(time.time())}"
    ...
    for pair_idx in range(num_turns):
        # ── USER AGENT turn — principle-adherence reminder ─────────────────────
        ...
        # ── CHATBOT AGENT turn — counseling principle-adherence reminder ───────
...
def pair_filename(scenario_id: int, emotion: str, agent_type: str, run_ts: str) -> str:
    ...
    return f"roleplay_doh_dual_noai_{scenario_id:04d}_{safe_emotion}_{agent_type}_{run_ts}.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Roleplay-doh Dual Adherence (Non-AI-Aware): principle-adherence for both agents, counseling session framing."
    )
    ...
    _log(f"Roleplay-doh Dual Adherence (Non-AI-Aware) simulation: {len(work_list)} pair(s)")
